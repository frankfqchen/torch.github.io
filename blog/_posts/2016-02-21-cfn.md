...
In the backward pass, the same idea can be applied. When no rating exist, the error is set to zero as it cannot be computed.
 ...
 ![training1](http://florian-strub.com/tmp/training1-small.png)
...
The autoencoder is also diverted from its original purpose. It does not aim to reconstruct the initial input anymore, it aims at predicting the missing data.

By keeping this idea in mind, we applied the Denoising loss [2] to empahisize the prediction aspect during the training.
A few input ratings are masked on every input vector, thus, it simulates missing ratings.
Finally, a higher error is provided when the output rating is erroneously predicted.
...
The loss is based on two main hyperparameters Alpha, Beta. They balance whether the Network would focus on denoising the input (alpha) or reconstructing the input (beta).
...
To avoid memory explosition, data must be stored in sparse tensors. Given the toy matrix in the first section, the sparse matrix of users/items would be:

 ![training1](http://florian-strub.com/tmp/dense2sparse.png)


The following code parse the movieLens-1M dataset and turn it into a sparse reprensation of items
...
Unbias the training mean by lines/columns increase the final results.   
...
We will use on the latter since we are want to focus on the prediction aspect.
This is the most verbose part but we try to keep the code as simple as possible. 
...
```lua
   ...
   local shuffle = torch.randperm(#train)
   ...
   for _, input in pairs(minibatch) do
	  ...
	  sgdConfiguration.evalCounter = currentEpoch
...
Torch criterions  do not handle Sparse target by default. Thus, it is necessary to wrap them into another criterion.
...
```lua
   ...
   for _, oneBatch in pairs(minibatch) do
	  local output = self.network:forward(oneBatch.input)
...
As a benchmark, Alternative Least squares is a widely used Collaborative algorithm. It has a final RMSE around 0.850 on movieLens-1M - cf [Mahout](http://mahout.apache.org/) + [Script](https://github.com/apache/mahout/blob/master/examples/bin/factorize-movielens-1M.sh)

Furthermore, the bigger is the dataset, the better will be the Autoencoder. State of the art results can be computed with movieLens-10/20M.
