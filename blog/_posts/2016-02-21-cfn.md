...
It turns out that a well-defined Autoencoder can perform among the best Collaborative Fitering algorithms. 
This post is focused on understating and implementing such Autoencoders.
...
For instance, if we gather the feedback of movies of all users into a matrix, we obtain a sparse matrix of ratings. Note that sparsity is induced by unknown values.
...
The Autoencoder is fed with these sparse vectors; its ouput will finally complete the line/column one after the other.
...
A more advanced description is described in our paper [4]. 
Importantly, we explain in tis paper the underlying idea that links classic Collaborative Filtering techniques and Autoencoders that __may explain__ why neural Networks performs well in Collaborative Filtering tasks. 
...
4. *Strub, F., Mary, J., Gaudel R., (2016). Collaborative Filterings and Neural Networks. Under review*
